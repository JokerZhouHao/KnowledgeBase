...
public class KSPIndex {
	...
	public KSPIndex(RTreeWithGI rgi, Set<Integer>[] rtreeNode2Pid,
				...
				if (first.level >= 0) {// node
					...
					for (int cChild = 0; cChild < n.m_children; cChild++) {
						...
						if (n.m_level == 0) {
							...
							minDateSpanMap = this.getWidMinDateSpan(Boolean.TRUE, nid, sortQwords, date);
...
//					"timeCptGetSemanticTree = " + Global.rr.timeCptGetSemanticTree/1000);
	...
	private Boolean placeReachablePrune(int place, int[] sortQwords) {
		...
		for(int i=0; i<sortQwords.length; i++) {
			if(!w2pReachable[i].contains(sortQwords[i]))	return Boolean.TRUE;
		}
	 ...
	 * @throws IOException
	 */
	public HashMap<Integer, Integer> getWidMinDateSpan(Boolean testReachable, int id, int[] sortQwords, int date) throws IOException {
		...
		for(i=0; i<sortQwords.length; i++)
			...
			if(testReachable)	{
				...
				widMinDateSpan.put(wid, wid2DateNidPair[i].getMinDateSpan(date, id, w2pReachable[i]));
			}
			else 	widMinDateSpan.put(wid, wid2DateNidPair[i].getMinDateSpan(date));
		}
