...
public class SPBestOpt {
	 ...
	 * @throws Exception
	 */
	public SPBestOpt() throws Exception{
		// 各索引路径
		...
		String wIdInfinitePNIndex = Global.outputDirectoryPath + Global.indexWidPN + "_" + String.valueOf(Global.radius) + "_" + Global.INFINITE_PN_LENGTH_STR + File.separator;
		...
		if(Global.MAX_PN_LENGTH> 0) {
			...
			wIdInfinitePnSer = new IndexWordPNService(wIdInfinitePNIndex);
			wIdInfinitePnSer.openIndexReader();
		}
		...
		for(i=0; i<sortQwords.length; i++) {
			...
			for(DateNidNode dnn : dnList) {
				...
				if(null == (dws = searchedDatesWids[dnn.getNid()])) {
					if(dnn.getDate() == Integer.MAX_VALUE)	dws = new DatesWIds(maxDate, sortQwords.length);
...
//			}
		...
		if(Global.optMethod == OptMethod.O2 || Global.optMethod == OptMethod.O4) {	// O2优化
			w2pReachable = new Map[sortQwords.length];
			for(i=0; i<sortQwords.length; i++) {
...
//					System.out.println("pn_len : " + bs.length);
		...
		KSPIndexOpt kSPExecutor = new KSPIndexOpt(rgi, rtreeNode2Pid, pid2RtreeLeafNode, cReach, searchedDatesWids, wid2DateNidPair, minMaxDateSer, w2pReachable, wordPNMap, maxDateSpans, signInRange);
		...
		if(args.length>0) {
			...
			if(args.length > 9) {
				...
				case 0:
					...
					break;
				case 1:
					Global.optMethod = OptMethod.O1;
					break;
				case 2:
					Global.optMethod = OptMethod.O2;
					break;
				case 3:
					Global.optMethod = OptMethod.O3;
					break;
				case 4:
			...
			sampleResultFile = sampleResultFile + "." + Global.optMethod;
		}
		
		if(Global.isTest) {
			Global.startTime = System.currentTimeMillis();
			System.out.println("> SP_BEST_OPT 开始测试样本 nwlen=" + String.valueOf(Global.MAX_PN_LENGTH) + " " +   
								"mds=" + String.valueOf(Global.maxDateSpan) + " " + 			
								"t=" + String.valueOf(searchType) + " " +
								"ns=" + String.valueOf(numSample) + " " +
								"r=" + String.valueOf(radius) + " " +
