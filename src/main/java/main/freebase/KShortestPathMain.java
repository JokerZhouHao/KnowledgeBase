...
import processor.freebase.KShortestPathsProcessor;
...
public class KShortestPathMain {
	...
	public static void main(String[] args) {
		...
		while(true) {
			 ...
			 * 开始初始化工作
			 *******************/
			System.out.println("> 开始初始化工作 . . . " + TimeUtility.getTime() + " " + MemoryInfo.getTotalFreeUsedAvailable() + "\n");
			KShortestPathsProcessor processor = new KShortestPathsProcessor(indexPath, curDegreeNum, curNodeNum);
			processor.init();
			System.out.println("> 完成初始化工作！！！" + TimeUtility.getTime() + " " + MemoryInfo.getTotalFreeUsedAvailable() + "\n");
...
//				}
				...
				System.out.println("> 共获得" + list.size() + "个shortest点," + TimeUtility.getTime() + " : ");
				if(list.size()!=0) {
					i = 0;
				...
				// 释放内存
				System.out.println("\n> 查找shortest的" + k + "个点结束, " + TimeUtility.getTime() + ", 花时(包含查词时间) : " + TimeUtility.getSpendTimeStr(startTime, System.currentTimeMillis()));
			}
			
			
		}
		System.out.println("\n> 退出成功  ! ! ! " + TimeUtility.getTime());
	}
}
