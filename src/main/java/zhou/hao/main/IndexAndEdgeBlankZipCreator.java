...
//		}
	 ...
	 */
	public void testZip() {
		ZipReaderService zipReader = new ZipReaderService(LocalFileInfo.getBasePath() + File.separator + "data" + File.separator + "Google-freebase-rdf-latestZH" + File.separator + "edgeBlank.zip");
		zipReader.readLine();
		zipReader.readLine();
	...
	public void createIndex() {
		...
		ZipReaderService reader = new ZipReaderService(LocalFileInfo.getBasePath() + File.separator + "data" + File.separator + "Google-freebase-rdf-latestZH" + File.separator + "keywordBlank.zip");
		reader.readLine();
	...
	public void writeEdgeBlankZip() {
		...
		File tempFile = null;
		ZipWriterService zipWriter = null;
		...
		for(i=0; i<tempLen; i++) {
			...
			if(tempFile.exists()) {
				System.out.println("  > 第" + (i+1) + "个zip已存在 ! ! ! " + TimeStr.getTime() + "\n");
				continue;
			}
			...
			if(tempDegreeNum ==0){	// 没有设置度
				// 开始写zip
				zipWriter = new ZipWriterService(tempZipPath);
				// 写正向边
...
//		creatorT.testZip();
		...
		System.out.println("> 完成处理 ! ! ! " + TimeStr.getTime());
	}
}
