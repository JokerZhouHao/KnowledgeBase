...
import zhou.hao.service.GoogleFreebaseLineProcessService.Node;
import zhou.hao.service.GoogleFreebaseMapWriteService;
import zhou.hao.service.NodeIdHashMapWriteServiceList;
import zhou.hao.service.NodeIdHashMapWriteServiceMap;
...
//					this.nodeIdHashMapTest.displayAllNodes();
					...
					if(precessedLineNum>tempProcessedLineNum) {
						System.out.println("> 已处理 " + precessedLineNum + "条, 完成10000%%, 已生成" + nodeIdHashMapTest.getNodeNum() + "个节点, 当前时间：" + TimeStr.getTime() + ", 内存占用情况如下：");
				...
				if(0 != lineList[sign].getWritePosition()) {
					for(Integer i=0; i<lineList[sign].getWritePosition(); i++) {
						...
						if((precessedLineNum-tempProcessedLineNum)==processedLineInBlock) {
							...
							Long nowTime = System.nanoTime();
							System.out.println("> 已处理 " + precessedLineNum + "条, 完成" + precessedLineNum*10000/totalLine + "%%, 已生成" + nodeIdHashMapTest.getNodeNum() + "个节点, 当前时间：" + TimeStr.getTime() + ", 内存占用情况如下：");
							System.out.println("  " + MemoryInfo.getTotalFreeUsedAvailable());
							System.out.println("  共用: "+ (nowTime - startTime)/1000000000/3600 + "h" + (nowTime - startTime)/1000000000%3600/60 + 
									"m" + (nowTime - startTime)/1000000000%3600%60 + "s");
							System.out.println("  现开始输出文件keywordAndEdge-" + hasProcessedBlock + ".zip. . .");
