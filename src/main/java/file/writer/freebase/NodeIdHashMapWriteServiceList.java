...
public class NodeIdHashMapWriteServiceList {
	private NodeIdHashMapTestList nodeHashMap = null;
	...
	public NodeIdHashMapWriteServiceList(NodeIdHashMapTestList ni) {
		this.nodeHashMap = ni;
	}
	...
	public boolean writeKeywordAndEdgeBlock(int num) {
		...
		try {
			...
			HashMap<Integer, ArrayList<String>> keywordMap = this.nodeHashMap.getKeywordHashMap();
			int i=0, j=0;
			int nodeNum = this.nodeHashMap.getNodeNum();
			ArrayList<String> keywordList = null;
			bw.write(nodeNum + "\n"); // 写入节点数
			for(i=0; i<nodeNum; i++) {
				keywordList = keywordMap.get(i);
				if(null==keywordList)	bw.write('\n');
				else {
			...
			HashMap<Integer, ArrayList<Integer>> edgeMap= this.nodeHashMap.getPointToNodeHashMap();
			ArrayList<Integer> edgeList = null;
			bw.write(nodeNum + "\n"); // 写入节点数
			for(i=0; i<nodeNum; i++) {
				edgeList = edgeMap.get(i);
				if(null==edgeList)	bw.write('\n');
				else {
					StringBuffer sb = new StringBuffer();
					for(Integer in : edgeList) {
						sb.append(in);
						sb.append(',');
					}
					sb.append('\n');
		...
		public void run() {
			...
			synchronized (WriteFileThead.class) {
				...
				if(--currentThreadNum<=0) {
					...
					System.out.println("  共处理 " + precessedLineNum + "条, 共生成" + nodeNum + "个节点, 当前时间: " + TimeUtility.getTime());
...
//				System.out.println(readLen);
	...
	class KeywordOneBlock extends OneBlock{
		...
		public ArrayList<String> readLineToList(int readedNodeId, ArrayList<String> keywordList){
					...
					if(str.isEmpty()) 	return null; 	// 该行没值
						...
						if(null==keywordList) {
							...
							for(int i=0; i<strArr.length; i++)
								keywordList.add(strArr[i]);
						} else {
							for(int i=0; i<strArr.length; i++) {
								if(!keywordList.contains(strArr[i]))
									keywordList.add(strArr[i]);
							}
						}
						return keywordList;
	...
	class EdgeOneBlock extends OneBlock{
		...
		public ArrayList<Integer> readLineToList(int readedNodeId, ArrayList<Integer> edgeList){
			if(readedNodeId>=this.nodeNum)	{	// 已读完
				...
				try {
					...
					if(str.isEmpty())	return null; 	// 该行没值
						...
						if(null==edgeList) {
							...
							for(int i=0; i<strArr.length; i++)
								edgeList.add(Integer.parseInt(strArr[i]));
						} else {
							int x = 0;
							for(int i=0; i<strArr.length; i++) {
	...
	public boolean writeKeywordFile() {
		...
		while(hasProcessedBlock<blockNum) {
			ArrayList<String> keyList = keywordBlocks[hasProcessedBlock].readLineToList(currentNodeId, null);
			if(null==keyList) {
				keyList = new ArrayList<String>();
			} else {
				if(keyList.isEmpty()) { // 该节点已读完
...
//					mergeArrayList(keyList, strList);
				keywordBlocks[i].readLineToList(currentNodeId, keyList);
			}
			// 写入当前节点的关键字
			try {
				if(!keyList.isEmpty()) {
	...
	public boolean writeEdgeFile() {
		...
		while(hasProcessedBlock<blockNum) {
			ArrayList<Integer> edgeList = edgeBlocks[hasProcessedBlock].readLineToList(currentNodeId, null);
			if(null==edgeList) {
				edgeList = new ArrayList<Integer>();
			} else {
				if(edgeList.isEmpty()) { // 该节点已读完
...
//					mergeArrayList(edgeList, intList);
				edgeBlocks[i].readLineToList(currentNodeId, edgeList);
			}
			// 写入当前节点的关键字
			try {
				if(!edgeList.isEmpty()) {
