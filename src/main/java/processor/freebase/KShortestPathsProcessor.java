...
import entity.freebase.AList;
import entity.freebase.FreeBaseMap;
import entity.freebase.MMap;
import entity.freebase.PathDistanceList;
...
//	private FreeBaseMap freeBaseMap = new FreeBaseMap(LocalFileInfo.getBasePath() + "orginal_code\\data\\testedges.zip");
	...
	public LinkedList<CanPathList> findKShortestPaths(int k, ArrayList<String> searchedWords){
		 ...
		 System.out.println("  > 开始查找word . . . " + TimeUtility.getTime());
		 ...
		 System.out.println("  > 所查词均在map中！" + TimeUtility.getTime() + "\n");
		  ...
		  *
		  ******************************/
		System.out.println("  > 初始化aList . . . " + TimeUtility.getTime());
...
//				recordMap.put(in, Boolean.TRUE);
		...
		System.out.println("  > 完成初始化aList ！！！ " + TimeUtility.getTime() + "\n");
		 ...
		 *  
		 ***********************************************/
		System.out.println("  > 初始化mmap . . . " + TimeUtility.getTime());
...
//					recordMap.put(in, Boolean.TRUE);
			...
			if(k == mmap.initAddPath(polledPath.getPathListIndex(), path)) {
				System.out.println("  > 初始化mmap成功！！！" + TimeUtility.getTime() + "\n");
				System.out.println("  > 已获得" + k + "条长度为0的最短路径！！！");
				System.out.println("  > 用时(不包含查词时间) : " + TimeUtility.getSpendTimeStr(startTime, System.currentTimeMillis()) + "\n");
		 ...
		 *************************************/
		int frontDis = -1;
		System.out.println("  > 开始计算" + k + "条候选路径.....  " + TimeUtility.getTime());
		boolean isNull = true;
...
//					recordMap.put(in, Boolean.TRUE);
			...
			if(aList.getHeadsHasChange()) {
				...
				if(longestDis!=frontDis) {
					frontDis = longestDis;
					System.out.println("    > 当前aList首元素长度和为  : " + longestDis + " " + MemoryInfo.getTotalFreeUsedAvailable() + " " + TimeUtility.getTime());
					System.out.print("        > ");
		...
		System.out.println("  > 已获得" + mmap.getCanShortestPathList().size() + "条候选路径！！！  " + TimeUtility.getTime());
		...
		System.out.println();
		if(isNull) {
			System.out.println("  > 队列为已为空，候选路径都是最短路径，获得" + mmap.getCanShortestPathList().size() + "条最短路径！！！" + TimeUtility.getTime());
			System.out.println("  > 用时(不包含查词时间) : " + TimeUtility.getSpendTimeStr(startTime, System.currentTimeMillis()) + "\n");
			return mmap.getCanShortestPathList();
		}
		 ...
		 *  
		 **************************************/
		System.out.println("  > 开始从候选路径中计算" + k + "条最短路径.....  " + TimeUtility.getTime());
		while((polledPath = aList.poll())!=null) {
			path = polledPath.getPath();
			pIndex = polledPath.getPathListIndex();
//			targetNode = path.getTargetNodeId();
...
//					recordMap.put(in, Boolean.TRUE);
			...
			if(mmap.addPath(polledPath.getPathListIndex(), path)) {
				System.out.println("  > 已获得" + k + "条最短路径！！！" + TimeUtility.getTime());
				System.out.println("  > 用时(不包含查词时间) : " + TimeUtility.getSpendTimeStr(startTime, System.currentTimeMillis()) + "\n");
				return mmap.getCanShortestPathList();
			}
		}
		
		System.out.println("  > 已获得" + mmap.getCanShortestPathList().size() + "条最短路径！！！" + TimeUtility.getTime());
		System.out.println("  > 用时(不包含查词时间) : " + TimeUtility.getSpendTimeStr(startTime, System.currentTimeMillis()) + "\n");
		return mmap.getCanShortestPathList();
	}
	
	public void initTermArr() {
		System.out.println("  > 开始初始化termArr . . . " + TimeUtility.getTime());
		searcher.openIndexReader();
		termArr = this.searcher.getTermArr(termArr);
		searcher.closeIndexReader();
		System.out.println("  > 完成初始化termArr ！！！" + TimeUtility.getTime());
	}
