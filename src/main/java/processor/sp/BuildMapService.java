...
//	private ArrayList<Integer>[] yago2sArrMap = new ArrayList[4];
	...
	public ArrayList<Integer>[] buildMap(){
		...
		System.out.println("> 开始构造Yago2sMap(" + yago2sArrMap.length + "个节点) . . .  " + TimeUtility.getTime());
		ZipBase64Reader ser = new ZipBase64Reader(zipPath, entryName);
		ser.readLine();
		String lineStr = null;
		int i, j, k, nodeId;
		...
		ser.close();
		System.out.println("> 结束构造Yago2sMap(" + yago2sArrMap.length + "个节点), 花时" + TimeUtility.getSpendTimeStr(startTime, System.currentTimeMillis()) + "  ！ ！！"+ TimeUtility.getTime() + "\n");
		return yago2sArrMap;
	}
