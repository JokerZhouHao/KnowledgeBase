	...
	public void run() {
		while(!this.endSign) {
			...
			System.out.println("> 已处理: " + lineConsumer.getPrecessedLineNum()*10000/this.lineConsumer.getTotalLine() + "%%, 共" + lineConsumer.getPrecessedLineNum()
					+ "行，已生成" + lineConsumer.getNodeNum() + "个节点, 当前时间, " + TimeUtility.getTime() + ", 共用时 : " + (nowTime - lineConsumer.getStartTime())/1000000000/3600 + "h" + (nowTime - lineConsumer.getStartTime())/1000000000%3600/60
