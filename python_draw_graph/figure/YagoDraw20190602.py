...
class Bar:
    ...
    def draw_radius_len(search_type=0, base_y = 600, ftype=None, fpath='test.pdf'):
        ...
        if search_type==0:
            ys = [1500+i*500 for i in range(0, 6)]
        elif search_type==1:
        ...
        for radiu_i in range(len(radius)):
            ...
            for len_i in range(len(lens)):
                data = Data.getData(fp=PathUtility.sample_res_path(base_dir, sp='SPBest', nwlen=lens[len_i], mds=50000000, t=search_type, ns=200, r=radius[radiu_i], k=5, nw=3, wf=50, dr=7, opt="O5"), time_total_threshold=12000000)
                print(data)
...
class LineChart:
    ...
    def draw_opt(show_type, base_y=None, fpath='test.pdf'):
        ...
        if show_type==0:
            # xLabel = r'opt-$SPTD$'
            ys = [7000 + i*2000 for i in range(8)]
        elif show_type==1:
            # xLabel = r'opt-$SPTR$'
    ...
    def draw_word_frequency(title=r'$\tau$', search_type=0, base_y=None, rotation=0, fpath='test.pdf'):
        if base_y!=None:
            if search_type==0:
                ys = [2000+i*2000 for i in range(0, 6)]
            elif search_type==1:
