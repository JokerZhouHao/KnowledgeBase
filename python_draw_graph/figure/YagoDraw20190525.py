...
class Bar:
    ...
    def draw_radius_len(search_type=0, base_y = 600, ftype=None, fpath='test.pdf'):
        ...
        if search_type==0:
            ys = [5000+i*2000 for i in range(0, 5)]
        elif search_type==1:
            ys = [5000+i*2000 for i in range(0, 5)]
        ...
        base_dir = 'D:\\nowMask\\KnowledgeBase\\sample_result\\yago2s_single_date\\20190524\\r_len\\'
        radius = [1, 2, 3]
...
class LineChart:
    ...
    def draw_word_frequency(title=r'$\tau$', search_type=0, base_y=None, rotation=0, fpath='test.pdf'):
        if base_y!=None:
            if search_type==0:
                ys = [10000+i*2000 for i in range(0, 15)]
            elif search_type==1:
        ...
        base_dir = 'D:\\nowMask\\KnowledgeBase\\sample_result\\yago2s_single_date\\20190524\\wf\\'
        search_name = None
        if search_type==0:
            search_name = r'$SPTD^*$'
        else:
        ...
        for search_type_index in range(len(search_types)):
            runtimes = []
            for wf in wfs:
                data = Data.getData(fp=PathUtility.sample_res_path(base_dir, sp='SPBest', nwlen=1000000, mds=50000000, t=search_types[search_type_index], ns=200, r=3, k=5, nw=3, wf=wf, dr=7), time_total_threshold=12000000)
                runtimes.append(data.timeTotal)
